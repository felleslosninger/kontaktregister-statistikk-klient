...
public class KontaktregisterScheduler {
    ...
    private static final String CRON_FIVE_MINUTE_INTERVAL = "0 */5 * * * *";
    ...
    public void fetchKontaktregisterD7Report() {
        ...
        final List<KontaktregisterFields> fields = asList(kontaktregisterFetch.perform(D7.getId(), startTime));
        ...
        logger.info(format("It took me %d", ((endTime.toInstant().toEpochMilli() - startTime.toInstant().toEpochMilli()) / 1000)));
    }
    ...
    public void fetchKontaktregisterD5Report() {
        ...
        final List<KontaktregisterFields> fields = asList(kontaktregisterFetch.perform(D5.getId(), startTime));
